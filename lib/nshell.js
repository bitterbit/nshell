...
const SIGINT = 'SIGINT'   // ctrl+c
const SIGSTP = 'SIGSTP'   // ctrl+z
const SIGCONT = 'SIGCONT' // resume after ctrl+z

Shell.prototype.initSignalHandlers = function(){
  process.on(SIGINT, function() {
    // Should never get here as readline catches signals
    debug("Caught interrupt signal from node");
  });

  var rl = this.rl;
  rl.on(SIGINT, function(){
    rl.write("^C")
    rl.clearLine(0); // clear entire line
    rl.write("\n");
  });

  // Continue when resuming after Ctrl+Z
  rl.on(SIGCONT, () => {
...
Shell.prototype.isBuiltin = function(name){
  return builtins.hasOwnProperty(name);
};

Shell.prototype.defualtPS1 = function(){
  var pwd = process.cwd() || this.env("PWD") || "";
  var sep = ' 𝕁𝕊 ▸ '
  return  pwd+sep;
};
...
Shell.prototype._process = function(cmds){
  var self = this;
...
Shell.prototype.lookup = function(cmds){
  for (var i = 0, len = cmds.length; i < len; ++i) {
    ...
    cmd.builtin = this.isBuiltin(cmd.name);
    if (cmd.builtin) continue;

    if (!cmd.path){
      var stat = safeStat(cmd.name);
      if (stat == undefined || !stat.isFile()){
        return this.missing(cmd);
      }
      cmd.path = cmd.name;
      cmd.name = cmd.name.split("/");
      cmd.name = cmd.name[cmd.name.length-1]; // take only the last part;
      console.log(cmd);
    }
...
Shell.prototype.exec = function(line){
  ...
  process.nextTick(function(){
    self.lastLine = line;
    if (e.defaultPrevented) return self.prompt();
    var cmds = utils.parse(e.line);
    self.process(cmds);
    debug('cmd %j', cmds);
    debug('env %j', cmds.env);
    if (!cmds.length) return self.prompt();
    if (!self.lookup(cmds)) return self.prompt();
    self.emit('exec', line);
    self.pipeline(cmds);
